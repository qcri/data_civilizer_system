...
def write_clustered_csv(c_data):
    keys = c_data[0].keys()
    with open('mycsvfile.csv', 'w') as output_file:
...
# def createClusters(table1, table2, column, data):
...
df=pd.read_csv("predictions.csv", sep=',', header=None)
data = df.values
...
write_clustered_csv(clustered_data)
