...
void OD::detect_outliers(TableProfile & TP, 
	...
	double mean, std, min_val = 0, max_val = 0;
	long L_Str, L_Nums;
	for (long i = 0; i < TP.header.size(); i++){
		...
		if (L_Str < num_non_ignorable_strings){
			...
			if(L_Nums > 0){
				min_val = dbl_itr->first; 	
				max_val = dbl_itr->first;
			}
			while (dbl_itr != TP.profile[i].distinct_Numbers.end()){
				numeric_data[double(dbl_itr->first)] = dbl_itr->second;
				min_val = MIN(min_val, dbl_itr->first);
				max_val = MAX(max_val, dbl_itr->first);
				dbl_itr ++;
			}		
			if (numeric_data.size() >= least_num_values){
				...
				sus_disg = DE.density_based_od(TP.header[i], numeric_data, TP.profile[i].common_Strings, std, min_val, max_val);
				// ================ For testing the density estimator =======================
